...
def get_buildsteps(codebases):
    ...
    libsodium_dir = Interpolate('%(prop:workdir)s/build/libsodium')
    libzmq_dir = Interpolate('%(prop:workdir)s/build/libzmq')
    czmq_dir = Interpolate('%(prop:workdir)s/build/czmq')
    pyzmq_dir = Interpolate('%(prop:workdir)s/build/pyzmq')
    pyczmq_dir = Interpolate('%(prop:workdir)s/build/pyczmq')
    ...
    env = {
        "LD_LIBRARY_PATH": Interpolate('%(prop:workdir)s/build/install/lib'),
        "PYTHONPATH": Interpolate(
    ...
    f.addStep(SetPropertyFromCommand(
        command="grep -c ^processor /proc/cpuinfo", property="NCPU"))

    ##########################################################################
    # Remove the artefacts from the last run
    f.addStep(RemoveDirectory(
        dir=Interpolate('%(prop:workdir)s/build/install')))

    # Create a new artefacts dir for this build
    f.addStep(MakeDirectory(
        dir=Interpolate('%(prop:workdir)s/build/install')))
    ...
    f.addStep(ShellCommand(
        ...
        command=["./configure", Interpolate(
            '--prefix=%(prop:workdir)s/build/install')]))

    f.addStep(ShellCommand(
        env=env,
        workdir=libsodium_dir,
        description=["libsodium", "build"],
        command=makeCommand))

    f.addStep(ShellCommand(
        env=env,
        workdir=libsodium_dir,
        description=["libsodium", "testing"],
        command=makeCheckCommand))
    ...
    f.addStep(ShellCommand(
        ...
        command=["./configure", "--with-pgm", "--with-libsodium",
                 Interpolate('--prefix=%(prop:workdir)s/build/install')]))
    ...
    f.addStep(ShellCommand(
        env=env,
        workdir=libzmq_dir,
        description=["libzmq", "build"],
        command=makeCommand))

    f.addStep(ShellCommand(
        env=env,
        workdir=libzmq_dir,
        description=["libzmq", "test"],
        command=["make", "check"]))

    f.addStep(ShellCommand(
        env=env,
        workdir=libzmq_dir,
        description=["libzmq", "install"],
        command=["make", "install"]))
    ...
    f.addStep(ShellCommand(
        env=env,
        workdir=czmq_dir,
        description=["czmq", "build"],
        command=makeCommand))

    f.addStep(ShellCommand(
        env=env,
        workdir=czmq_dir,
        description=["czmq", "test"],
        command=makeCheckCommand))

    f.addStep(ShellCommand(
        env=env,
        workdir=czmq_dir,
        description=["czmq", "install"],
        command=["make", "install"]))
    ...
    f.addStep(ShellCommand(
        ...
        command=["python", "setup.py", "configure",
                 Interpolate('--zmq=%(prop:workdir)s/build/install')]))
